...
// Main
// ---------------------------------------------------------------------------
const CSV_PATH = path.join(process.cwd(), "data", "gyms_eastern.csv");
const OUT_PATH = path.join(process.cwd(), "data", "gyms_eastern.json");

if (!fs.existsSync(CSV_PATH)) {
  console.error(`CSV not found: ${CSV_PATH}`);
  console.error("Run seedGymsCSV_eastern.ts first.");
  process.exit(1);
}
...
// Count existing WA gyms so IDs continue from where gyms.json left off
let existingCount = 0;
const waJsonPath = path.join(process.cwd(), "data", "gyms.json");
if (fs.existsSync(waJsonPath)) {
  const waGyms = JSON.parse(fs.readFileSync(waJsonPath, "utf-8"));
  existingCount = Array.isArray(waGyms) ? waGyms.length : 0;
  console.log(`Continuing IDs from gyms.json (${existingCount} existing gyms)`);
}
...
console.log(`\nWrote ${gyms.length} gyms to data/gyms_eastern.json`);
for (const [state, count] of Object.entries(byState)) {
  console.log(`  ${state}: ${count}`);
}
console.log(`\nNext step: review the JSON, then run scripts/seed.ts to upload to DynamoDB.\n`);
