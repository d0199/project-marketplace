...
// Output: CSV with Email, Business Name, Gym Category, is_enterprise
...
const OUTPUT = path.join(__dirname, "..", "data", "wa_gyms_email_list.csv");

const ddb = DynamoDBDocumentClient.from(
  new DynamoDBClient({ region: "ap-southeast-2" })
);

// Known franchise/enterprise brand patterns
const ENTERPRISE_PATTERNS = [
  ...
  /\bcrossfit\b/i,
  /\bbft\b/i,
  /\bbody fit training\b/i,
  /\bcurves\b/i,
  /\bgold'?s gym\b/i,
  /\bkx pilates\b/i,
  /\bgenesis\b/i,
  /\bspeedfit\b/i,
  /\bstudio pilates international\b/i,
  /\bpronto pilates\b/i,
  /\borangetheory\b/i,
  /\b9round\b/i,
  /\bclub lime\b/i,
  /\bfernwood\b/i,
  /\bviva leisure\b/i,
  /\bplatinum fitness\b/i,
  /\bfit college\b/i,
  /\borbit fitness\b/i,
  /\belite fitness\b/i,
  /\bbest body pilates\b/i,
  /\bshemoves\b/i,
  /\bflow space\b/i,
];
...
async function main() {
  ...
  // Build email frequency map for enterprise detection
  ...
  // Name frequency (normalised) for fuzzy enterprise detection
  ...
  function isEnterprise(g: Record<string, unknown>): boolean {
    ...
    // Known franchise patterns
    if (ENTERPRISE_PATTERNS.some((p) => p.test(name))) return true;

    // Same email used by multiple gyms
    if ((emailCount.get(email) || 0) > 1) return true;

    // Same email domain used by 3+ gyms (excludes gmail/outlook/yahoo)
    ...
    // Fuzzy name appears multiple times
  ...
  // Build CSV
  const header = "Email,Business Name,Gym Category,is_enterprise";
  const rows = waWithEmail.map((g) => {
    ...
    return [escapeCSV(email), escapeCSV(name), escapeCSV(specialties), enterprise].join(",");
  });
  ...
  const independentCount = waWithEmail.length - enterpriseCount;
