...
export default function ClaimModal({ gym, onClose, initialEmail = "", initialName = "" }: Props) {
  ...
  async function checkEmail(val: string) {
    if (!val || initialEmail) return; // skip if logged in
    try {
    ...
    try {
      const res = await fetch("/api/claim", {
        ...
        body: JSON.stringify({
          gymId: gym.id,
          gymName: gym.name,
          gymAddress: `${gym.address.street}, ${gym.address.suburb} ${gym.address.state} ${gym.address.postcode}`,
    ...
    >
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <div>
            <h2 className="text-lg font-bold text-gray-900">Claim this listing</h2>
            <p className="text-sm text-gray-500 mt-0.5">{gym.name}</p>
          </div>
          <button
            onClick={onClose}
        ...
        {status === "success" ? (
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="bg-gray-50 rounded-lg p-3 text-sm text-gray-600 space-y-0.5">
              ...
              <p>
                {gym.address.street}, {gym.address.suburb} {gym.address.state}{" "}
                {gym.address.postcode}
              </p>
              {gym.website && (
                <p className="text-brand-orange truncate">{gym.website}</p>
              )}
            </div>
            ...
            {initialEmail ? (
              ...
              <p className="text-xs text-gray-500 bg-gray-50 rounded-lg px-3 py-2">
                ...
                <Link href={`/owner?redirect=${encodeURIComponent(`/gym/${gym.id}?claim=true`)}`} className="text-brand-orange hover:underline font-medium">
                  Sign in first
                </Link>{" "}
            ...
            <div>
              ...
              {emailExists && (
                <div className="mt-1.5 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                  ...
                  <Link href={`/owner?redirect=${encodeURIComponent(`/gym/${gym.id}?claim=true`)}`} className="text-brand-orange hover:underline font-semibold">
                    Sign in
                  </Link>{" "}
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Tell us about your relationship to this gym…"
              />
            </div>
