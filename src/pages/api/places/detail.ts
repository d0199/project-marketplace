...
function extractField(components: AddressComponent[], type: string): string {
  return components.find((c) => c.types.includes(type))?.long_name ?? "";
}

function extractFieldShort(components: AddressComponent[], type: string): string {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (data.status === "OK" && data.result) {
      ...
      const suburb = extractField(components, "locality");
      const state = extractFieldShort(components, "administrative_area_level_1");
      const postcode = extractField(components, "postal_code");
      const lat = data.result.geometry?.location?.lat ?? null;
      const lng = data.result.geometry?.location?.lng ?? null;

      return res.json({ street, suburb, state, postcode, lat, lng, formattedAddress: data.result.formatted_address ?? "" });
    }
